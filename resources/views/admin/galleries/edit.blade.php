...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.galleries.update", [$gallery->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="worked_by">{{ trans('cruds.gallery.fields.worked_by') }}</label>
                <input class="form-control {{ $errors->has('worked_by') ? 'is-invalid' : '' }}" type="text" name="worked_by" id="worked_by" value="{{ old('worked_by', $gallery->worked_by) }}">
                @if($errors->has('worked_by'))
                    <div class="invalid-feedback">
                        {{ $errors->first('worked_by') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.gallery.fields.worked_by_helper') }}</span>
            </div>
            <div class="form-group">
