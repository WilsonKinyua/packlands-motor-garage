...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.serviceOfferedBy.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-offered-bies.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('expert') ? 'is-invalid' : '' }}" name="expert_id" id="expert_id" required>
                    @foreach($experts as $id => $entry)
                        <option value="{{ $id }}" {{ old('expert_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
